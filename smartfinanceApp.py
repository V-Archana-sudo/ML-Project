...
# Dashboard second. All previous functionality preserved and bugs fixed.
...
def load_data() -> pd.DataFrame:
    if DATA_PATH.exists():
        try:
            ...
            # fallback: read without parsing and coerce later
...
# Always offer a download button (if df exists)
csv_bytes = st.session_state.df.to_csv(index=False).encode("utf-8")
st.sidebar.download_button("Export data (CSV)", data=csv_bytes, file_name="smartfinance_data.csv", mime="text/csv")
...
# Tabs in main area (Add Expense FIRST, Dashboard SECOND per user request)
...
with tab_add:
    ...
    with st.form("add_expense", clear_on_submit=True):
        ...
        if notes:
            ...
            if any(k in t for k in ["swiggy","zomato","restaurant","cafe","dine","pizza","burger"]):
                ...
                suggested = "Groceries"
        default_idx = 0
        if suggested and suggested in DEFAULT_CATEGORIES:
            default_idx = DEFAULT_CATEGORIES.index(suggested)
        category = st.selectbox("Category", options=DEFAULT_CATEGORIES, index=default_idx)
    ...
    if st.button("Train / Update auto-categorizer"):
        ...
        if len(data) < 30:
            ...
            try:
                ...
                st.warning("Could not save the model to disk (permission/file error). Model still available in memory for this session.")
    ...
    if st.session_state.df.empty:
        ...
        # reset index so row numbers shown match edit/delete indices
        df_display = st.session_state.df.copy().reset_index(drop=True)
        ...
        if len(df_display) > 0:
            ...
            new_amt = st.number_input("Correct amount (₹)", min_value=0.0, value=float(sel.Amount), key="edit_amt")
            new_cat = st.selectbox("Category", options=DEFAULT_CATEGORIES, index=DEFAULT_CATEGORIES.index(sel.Category) if sel.Category in DEFAULT_CATEGORIES else 0, key="edit_cat")
            if st.button("Update expense"):
                real_idx = int(idx)
                # update session df with positional index
            ...
            if st.button("Delete expense"):
                real_idx = int(idx)
                st.session_state.df = st.session_state.df.reset_index(drop=True).drop(real_idx).reset_index(drop=True)
                save_data(st.session_state.df)
...
with tab_dash:
    ...
    if st.session_state.df.empty:
        ...
        top_cat = "—"
        if (not month_df.empty) and month_df["Category"].notna().any():
            try:
                top_cat = month_df.groupby("Category")["Amount"].sum().sort_values(ascending=False).index[0]
            except Exception:
                top_cat = "—"
        total_budget = float(st.session_state.budgets.get("__total__", 0.0) or 0.0)
        total_cat_budget = sum(v for k,v in st.session_state.budgets.items() if k != "__total__")
        # prefer explicit total budget if set else sum of categories
        effective_budget = total_budget if total_budget > 0 else total_cat_budget
        util_pct = (total_spent / effective_budget * 100) if effective_budget > 0 else 0.0
        ...
        with c1:
            ...
            if not month_df.empty:
                ...
                # convert negative spends to positive for pie display (if data is negative)
                values = np.abs(cat_sum.values)
                if values.sum() == 0:
                    st.info("No spend amounts recorded this month.")
                else:
                    ax.pie(values, labels=cat_sum.index, autopct="%1.0f%%", startangle=90)
                    ax.axis("equal")
                    st.pyplot(fig)
            else:
        ...
        with c2:
            ...
            if not trend.empty and trend.sum() != 0:
...
with tab_budget:
    ...
    # total budget
    total_budget_val = st.number_input("Set total monthly budget (₹)", min_value=0.0, value=float(st.session_state.budgets.get("__total__", 0.0) or 0.0), step=500.0, key="total_budget_input")
    ...
    if not df.empty:
        ...
        for cat in DEFAULT_CATEGORIES:
            ...
            pct = (abs(spent) / limit * 100) if limit > 0 else 0.0
            tone = "ok" if pct < 70 else ("warn" if pct < 100 else "bad")
            st.markdown(f"**{cat}** — {money(spent)} / {money(limit)}  <span class='chip {('ok' if tone=='ok' else ('warn' if tone=='warn' else 'bad' if tone=='bad' else ''))}'>{pct:.0f}%</span>", unsafe_allow_html=True)
